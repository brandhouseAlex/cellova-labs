...
## Reference-aligned purchase panel
...
## Homepage gutters, provider media, and PDP refinements

The current main-branch homepage uses the shared `home-page-container` for the hero, Research Formats, Documentation First, and Product to Research modules. The requested section-start treatment uses Spark `#F2A63C` for **Research-Focused. Quality Driven.**, **Research Formats**, **Explore the catalog**, and **Documentation first**. Homepage and collection product grids render white card and section surfaces with Spark add-to-cart actions that retain white text in base and hover states.

Title-based image overrides have been removed from every product-card variant so product cards use the active commerce provider’s `featuredImage` or first provider image only; collection routes continue to query by their selected collection handle. The selected PDP provider image now retains Next Image’s required absolute fill positioning inside the existing square gallery frame; an erroneous `relative` utility was removed from the image itself so the primary media paints consistently. The flat COA tab exposes the Indigo Download COA action, its Storage Instructions tab uses the corrected cold-storage icon, and the Research Standards strip retains the requested slight blue-gray surface tint and shadow.

The GitHub main-branch Vercel homepage at revision `8f88813` displayed the requested Spark section-start labels, unified hero and Research Formats margins, white catalog-card surfaces, and the published **From product to research** four-step sequence below Documentation First. The corresponding active BPC-157 route rendered the provider-backed gallery frame, the white-text Spark add-to-cart action, the flat COA table content, and the corrected product-detail supporting controls. As with earlier product reviews, the primary provider image may complete after the initial paint; it is verified to become visible after the browser finishes its normal image request.

The follow-up main-branch gallery release at revision `1d0ae2e` removes the accidental `relative` utility from the filled primary media element. A repeat live BPC-157 review confirms that the provider-supplied image visibly paints within the unchanged outlined square gallery frame after the request completes. The correction is isolated to image positioning and does not alter the surrounding PDP layout, purchase panel, COA section, or product content.
