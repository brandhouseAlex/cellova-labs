...
Title-based image overrides have been removed from every product-card variant so product cards use the active commerce provider’s `featuredImage` or first provider image only; collection routes continue to query by their selected collection handle. An authenticated Vercel BPC-157 review confirms the selected provider image visibly loads in the main gallery after its normal media request completes. The flat COA tab exposes the Indigo Download COA action, its Storage Instructions tab uses the corrected cold-storage icon, and the Research Standards strip retains the requested slight blue-gray surface tint and shadow.
