...
The repository includes `vercel.json`, which runs `pnpm build`, publishes `dist/public` as the static storefront, sends `/api/*` to `api/index.ts`, and rewrites browser routes to the React application shell. The Vercel serverless entry uses the same Express/tRPC application factory as the local Node server, so commerce provider selection, OAuth routes, and API procedures remain consistent.
...
| 3 | Accept the repository `vercel.json` settings. Vercel runs `pnpm build`; the static storefront is `dist/public` and `/api/*` is a serverless function. | Do not substitute another storefront’s build settings. |
