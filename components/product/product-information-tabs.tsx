...
function CertificateOfAnalysis({ coas }: { coas: CommerceCertificateOfAnalysis[] }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[0.56fr_1fr]">
    <aside className="rounded-[12px] border border-[#D8DCE3] bg-gradient-to-br from-white via-[#F3F4F1] to-[#FFF1DB] p-7 shadow-[0_16px_32px_-26px_rgba(32,32,32,0.35)]">
    ...
    <div className="space-y-5">
      {coas.length > 1 ? <label className="flex flex-wrap items-center justify-between gap-3 rounded-[10px] border border-line bg-paper px-5 py-3.5 text-sm shadow-[0_12px_24px_-25px_rgba(32,32,32,0.35)]"><span className="font-semibold text-ink">Select lot</span><select value={selectedLot} onChange={(event) => setSelectedLot(Number(event.target.value))} className="rounded-[8px] border border-line bg-paper px-3 py-2 text-sm text-ink outline-none focus:border-brand">{coas.map((record, index) => <option key={`${record.lotNumber ?? "coa"}-${index}`} value={index}>{record.lotNumber ?? `COA ${index + 1}`}{record.testedDate ? ` — ${record.testedDate}` : ""}</option>)}</select></label> : null}
      <section className="overflow-hidden rounded-[12px] border border-[#D8DCE3] bg-paper shadow-[0_16px_32px_-26px_rgba(32,32,32,0.32)]" aria-labelledby="batch-information-heading">
        ...
        <dl className="divide-y divide-[#D8DCE3]">{batchFields.map(([label, value]) => <div key={label} className="grid grid-cols-[minmax(9rem,0.45fr)_1fr] gap-5 px-5 py-3 text-sm"><dt className="font-semibold text-ink">{label}</dt><dd className="min-w-0 break-words text-slate">{value || "Not provided"}</dd></div>)}</dl>
      </section>
